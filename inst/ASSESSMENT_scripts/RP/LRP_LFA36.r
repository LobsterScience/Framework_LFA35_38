...
ind36$oyr = ind36$year

ind36$year=ind36$oyr+1
...
###############################################################################################                            
                            ...
                            ggplot(as.data.frame(model),aes(b_Intercept,b_I1DsE))+
                              ...
                              scale_x_continuous(limits=c(0.028,.067))+
                            theme_bw()
                     ...
                     ##rerun based on EIV This step takes time
                            ...
                            ggplot(dd,aes(b_Intercept,b_I1DsE))+
                              geom_hex(bins = 70) +
                              scale_fill_continuous(type = "viridis") +
                              scale_x_continuous(limits=c(0.024,.069))+
...
#biomass and LRP
ggplot(subset(ind36),aes(x=year,y=estB/1000,ymin=lwrB/1000,ymax=uprB/1000))+geom_point()+geom_line()+geom_ribbon(alpha=.25)+
...
gll$relF = gll$LFA36/(gll$estB/1000)
gll$relF_l = gll$LFA36/(gll$lwrB/1000)
gll$relF_u = gll$LFA36/(gll$uprB/1000)
...
saveRDS(list(ind36,LRP_buff,g5,b,mL,gll),file='LFA36Index_RPs_Jan21_2026.rds')
