...
i = which(x$LONGITUDE>0)
if(length(i)>0) x$LONGITUDE[i] = x$LONGITUDE[i]*-1
...
ggLobsterMap('34-38',addGrids = F,addPoints = T,pts=subset(survey,YEAR %in% 2013:2024 ))


yp = ILTS_ITQ_All_Data(redo_base_data = F,size=c(0,300),aggregate=F,species=2550,biomass = F,extend_ts = F)
yp1 = subset(yp, month(SET_DATE) %in% 6:8 & YEAR<2025 & YEAR>2012)
...
ggplot(ns)+geom_sf()+geom_sf(data=z,colour='red')+geom_sf(data=ys,colour='blue')
