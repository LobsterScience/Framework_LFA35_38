...
#summer
y = ILTS_ITQ_All_Data(redo_base_data = F,size=c(70,300),aggregate=T,species=2550,biomass = F,applyGearConversion = T)
y = subset(y,month(SET_DATE)<8)
y$Survey='ILTS_Summer'
y$id = paste(y$TRIP_ID,y$SET_NO,sep="-")
y$N = y$SA_CORRECTED_PRORATED_N*y$sweptArea
y = subset(y,select=c(id,YEAR,SET_LONG,SET_LAT,N,Survey,sweptArea))
...
x$Survey = 'RV_Summer'
x = subset(x,select=c(id,YEAR,LONGITUDE,LATITUDE,N,Survey,OFFSET))
...
#fall
      ...
      z$YEAR = z$Year
      z = subset(z,select=c(id,YEAR,N,OFFSET))
      z$Survey = 'MNH_Fall'
      ...
      n = subset(n,select=c(id,YEAR,LONGITUDE,LATITUDE,N,OFFSET))
      n$Survey = 'NEFSC_Fall'
      ...
      y$Survey = 'ILTS_Fall'
      y$N = y$SA_CORRECTED_PRORATED_N*y$sweptArea
      y$OFFSET=y$sweptArea
      y = subset(y,select=c(id,YEAR,SET_LONG,SET_LAT,N,Survey,OFFSET))
      
      ys = st_as_sf(y,coords=c('SET_LONG','SET_LAT'),crs=4326)
      ys = subset(ys,select=c(id,YEAR,N,Survey,OFFSET))
...
combSUFall = subset(combSUFall, !is.na(N))
...
saveRDS(survey,file='survey_data_all_combined_jan182026_offset.rds')
