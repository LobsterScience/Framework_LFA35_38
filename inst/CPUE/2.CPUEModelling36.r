...
require(ggeffects)
require(ggforce)
fd = file.path(project.datadirectory('Framework_LFA35_36'),'outputs','CPUE')
setwd(fd)
...
l36 = gam(WEIGHT_KG~fYear+offset(leffort),data=subset(aT,LFA==36),family = Gamma(link='log'),method='REML')
l36a = gam(WEIGHT_KG~s(time)+fYear+offset(leffort),data=subset(aT,LFA==36),family = Gamma(link='log'),method='REML')
l36b = gam(WEIGHT_KG~s(time)+s(time,by=fYear)+fYear+offset(leffort),data=subset(aT,LFA==36),family = Gamma(link='log'),method='REML')

saveRDS(list(l36,l36a,l36b),'first3CPUEmodels36.rds')
w = readRDS('first3CPUEmodels36.rds')
l36 = w[[1]]
l36a = w[[2]]
l36b = w[[3]]

aT$rLID = as.factor(aT$LICENCE_ID)
l36c = gam(WEIGHT_KG~fYear+s(time,by=fYear)+s(rLID,bs='re')+offset(leffort),data=subset(aT,LFA==36),family = Gamma(link='log'),method='REML') #random intercept 
saveRDS(l36c,'CPUEmodel_LicenceRE36.rds')
l36c = readRDS('CPUEmodel_LicenceRE36.rds')


aT$rV = as.factor(aT$VR_NUMBER)
l36d = gam(WEIGHT_KG~fYear+s(time,by=fYear)+s(rV,bs='re')+offset(leffort),data=subset(aT,LFA==36),family = Gamma(link='log'),method='REML') # random intercept
saveRDS(l36d,'CPUEmodel_vesselRE36.rds')
l36d = readRDS('CPUEmodel_vesselRE36.rds')
# 
# 
# #plot index
# #these are conditional
# pr1 = ggpredict(l36,terms='fYear',condition = c(leffort=0))
# pr2 = ggpredict(l36a,terms=c('fYear'),condition = c(leffort=0, time=20))
# pr3 = ggpredict(l36b,terms='fYear',condition = c(leffort=0,time=20))
# pr4 = ggpredict(l36c,terms='fYear',condition = c(leffort=0,time=20))
# pr5 = ggpredict(l36d,terms='fYear',condition = c(leffort=0,time=20))
# 
...
# b = emmeans::emmeans(l36,~fYear,offset=0,type='response',data=subset(aT,LFA==36),at=list(time=c(1:60,146:242)))
# bmm = as.data.frame(summary(b))[c('response', 'lower.CL','upper.CL')]
# bmm$Model = 'Base'
# bmm$Year = 2006:2023
# d = emmeans::emmeans(l36a,~fYear,offset=0,type='response',data=subset(aT,LFA==36),at=list(time=c(1:60,146:242)))
# dmm = as.data.frame(summary(d))[c('response', 'lower.CL','upper.CL')]
# dmm$Model = 'BD'
# dmm$Year = 2006:2023
# f = emmeans::emmeans(l36b,~fYear,offset=0,type='response',data=subset(aT,LFA==36),at=list(time=c(1:60,146:242)))
# fmm = as.data.frame(summary(f))[c('response', 'lower.CL','upper.CL')]
# fmm$Model = 'BxD'
# fmm$Year = 2006:2023
# g = emmeans::emmeans(l36c,~fYear,offset=0,type='response',data=subset(aT,LFA==36),at=list(time=c(1:60,146:242)))
# gmm = as.data.frame(summary(g))[c('response', 'lower.CL','upper.CL')]
# gmm$Model = 'BxDRL'
# gmm$Year = 2006:2023
# 
# h = emmeans::emmeans(l36d,~fYear,offset=0,type='response',data=subset(aT,LFA==36),at=list(time=c(1:60,146:242)))
...
# write.csv(oo,'LFA36MarginalMeanCPUE.csv')
# 
# 
...
ind = aggregate(SD_LOG_ID~time+SYEAR,data=subset(aT,LFA ==36),FUN=function(x) length(unique(x)))
...
b = emmeans::emmeans(l36,~fYear,offset=0,type='response',data=subset(aT,LFA==36))
bmm = as.data.frame(summary(b))[c('response', 'SE')]
bmm$Model = 'Base'
bmm$SYEAR = 2006:2023
names(bmm)[1:2]=c('wemm','wse')

d = emmeans::emmeans(l36a,~fYear+time,offset=0,type='response',data=subset(aT,LFA==36),at=list(time=c(1:60,146:242)))
...
f = emmeans::emmeans(l36b,~fYear+time,offset=0,type='response',data=subset(aT,LFA==36),at=list(time=c(1:60,146:242)))
...
g = emmeans::emmeans(l36c,~fYear+time,offset=0,type='response',data=subset(aT,LFA==36),at=list(time=c(1:60,146:242)))
...
h = emmeans::emmeans(l36d,~fYear+time,offset=0,type='response',data=subset(aT,LFA==36),at=list(time=c(1:60,146:242)))
...
# pr2 = ggpredict(l36a,terms=c('fYear','time[1:235]'),condition = c(leffort=0), back_transform = F)
# pr2ta = data.frame(fYear=pr2$x,predicted=pr2$predicted,Model='BD',se=pr2$std.error,time=pr2$group)
# 
# 
# ##marginal weighted by effort
# 
# ind = aggregate(NUM_OF_TRAPS~time+SYEAR,data=subset(aT,LFA ==36),FUN=sum)
# ind1 = aggregate(NUM_OF_TRAPS~SYEAR,data=subset(aT,LFA ==36),FUN=sum)
...
# p2$wse = sqrt(p2$wvar)
# 
# pr3 = ggpredict(l36b,terms=c('fYear','time[1:235]'),condition = c(leffort=0))
...
# p3$wse = sqrt(p3$wvar)
# 
# pr4 = ggpredict(l36c,terms=c('fYear','time[1:235]'),condition = c(leffort=0))
...
# p4$wse = sqrt(p4$wvar)
# 
# pr5 = ggpredict(l36d,terms=c('fYear','time[1:235]'),condition = c(leffort=0))
...
# 
# #100 samples
# cv_df = crossv_mc(subset(aT,LFA==36), 50) 
# 
# #names samples
# cv_df =
#   cv_df |> 
