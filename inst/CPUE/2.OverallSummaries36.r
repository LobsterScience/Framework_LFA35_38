...
gr = readRDS(file.path(map_data,'GridPolyLand.rds'))
st_crs(gr) <- 4326
...
aT = readRDS(file.path(outdir,'CPUE','CPUETempDepth.rds'))
aT = subset(aT,SYEAR>2005 & SYEAR<2023 & LFA==36)
...
cc = merge(cc,preT)
cc = cc[order(cc$yr,cc$t),]
ggplot(subset(cc,lfa==36),aes(x=t,y=unBCPUE))+geom_point() + facet_wrap(~yr)
ggplot(subset(cc,lfa==36),aes(x=t,y=temp))+geom_point() + facet_wrap(~yr)
...
#modelling approach rather than relying on 'fixed' temperature relationship
#treating daily cpue as a point estimate
#not really the right thing to do so dont bother using, really only an exploration
