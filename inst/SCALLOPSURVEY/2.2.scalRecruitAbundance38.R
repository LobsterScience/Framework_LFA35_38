...
ci_results <- boot_results %>%
  ...
  mutate(
    ...
    ci_lower = if (!is.null(ci)) ci[4] else NA,  # Handle cases where CI cannot be calculated
    ci_upper = if (!is.null(ci)) ci[5] else NA   # Handle cases where CI cannot be calculated
  ) %>%
