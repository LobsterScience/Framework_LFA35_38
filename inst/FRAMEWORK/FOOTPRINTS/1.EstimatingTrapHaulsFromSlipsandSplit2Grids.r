...
a = subset(a,SYEAR>2004 & SYEAR<2025 & LFA %in% c(34,35,36,38))
b = lobster.db('seasonal.landings')
 b = subset(b,!is.na(SYEAR))
b$SYEAR = as.numeric(substr(b$SYEAR,6,9))
b$LFA38B <- NULL
b0 =b = subset(b,SYEAR>2004 & SYEAR<2025)
b = reshape(b,idvar='SYEAR', varying=list(3:6),direction='long')
b$LFA=rep(c(34,35,36,38),each=nrow(b0))
b$time <- NULL
...
uni = uu[which(uu %ni% pl$PORT_CODE )]
 ...
 ###################
 #### logs by grid for first part of seasons
 
 
 g = lobster.db('process.logs')
 g = subset(g,SYEAR>2004 & SYEAR<2025 & LFA %in% c (35,36,38)) 
 g$m = lubridate::month(g$DATE_FISHED)
 gKL = aggregate(WEIGHT_KG~LFA+GRID_NUM+SYEAR,data =subset(g,m %in% c(10,11,12,1)),FUN=sum)
 
 saveRDS(gKL,'DiscretizedData/LoglandingsFirsttoJan.rds')
