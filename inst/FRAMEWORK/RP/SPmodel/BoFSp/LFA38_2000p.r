...
bdm38 = ggplot(dd,aes(x=N,y=fit,ymin=lower,ymax=upper))+geom_point()+geom_ribbon(alpha=.25)+geom_path()+theme_test(base_size = 14)+labs(x='Year',y='Biomass')+geom_hline(yintercept=a$median$BMSP2*0.4, colour='blue',linewidth=1.2)
...
gridExtra::grid.arrange(bdm35,bdm36,bdm38,ncol=2,layout_matrix=rbind(c(1,2),c(3,4)))
