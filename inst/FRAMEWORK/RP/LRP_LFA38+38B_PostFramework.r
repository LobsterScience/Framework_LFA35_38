...
ggplot(subset(ind35,year>=2000 &year<2024),aes(year,estB/1000,ymin=lwrB/1000,ymax=uprB/1000))+geom_point()+geom_ribbon(alpha=.25)+geom_path()+
  geom_ribbon(data=subset(ind35F,year<2024),aes(year, estB/1000, ymin=lwrB/1000,ymax=uprB/1000),alpha=.25,fill='red')+geom_point(data=subset(ind35F,year<2024),aes(year, estB/1000),pch=1)+geom_path(data=subset(ind35F,year<2024),aes(year, estB/1000),linetype='dashed')+
  theme_test(base_size = 14)+labs(x='Year',y='Commercial Biomass', title='LFA35') +geom_hline(yintercept = min(ind35$estB/1000),col='blue',linewidth=1.3)+
...
gl = subset(gl,select=c(year,LFA38,LFA38B))
gl$LFA38B[which(gl$year==2002)] = g8$MT[which(g8$year==2002)]
gl$LFA38B[which(gl$year==2003)] = g8$MT[which(g8$year==2003)]
gl$LFA38B[which(gl$year==2004)] = g8$MT[which(g8$year==2004)]
gl$LFA38B[which(gl$year==2005)] = g8$MT[which(g8$year==2005)]
gl = na.zero(gl)
gl$LFA38 = gl$LFA38+gl$LFA38B
gl = subset(gl,year>0,select=c(year,LFA38))
...
ggplot(subset(gg, year<2024), aes(x = year)) +
  ...
  guides(fill = "none", color = "none") +theme_test(base_size = 14)
...
ggplot(as.data.frame(model),aes(b_Intercept,b_I1DsE))+
  ...
  scale_x_continuous(limits=c(0.011,.03))+
theme_bw()
...
ggplot(g5, aes(est1000, MEAN_T)) + geom_point(size=2)+geom_errorbarh(data=g5, aes(xmin= lwrB/1000,xmax=uprB/1000),linetype='dotted')+theme_test()+
...
ggplot(dd,aes(b_Intercept,b_I1DsE))+
  geom_hex(bins = 70) +
  scale_fill_continuous(type = "viridis") +
  scale_x_continuous(limits=c(0.01,.042))+
...
ggplot(g5, aes(est1000, MEAN_T)) + geom_point(size=2)+geom_errorbar(data=g5, aes(est1000, ymin=lCI,ymax=uCI),linetype='dotted')+geom_errorbarh(data=g5, aes(xmin= lwrB/1000,xmax=uprB/1000),linetype='dotted')+theme_test()+
  geom_line(data=tos,aes(x=sE,y=Pred,group=i),colour='blue',alpha=.005) +labs(x='Survey Commercial Biomass',y='Landings per Vessel')+theme_test(base_size = 14)
...
#EIV
ggplot() + geom_point(data=g5, aes(x=est1000,y= MEAN_T),size=2)+geom_errorbarh(data=g5, aes(y=MEAN_T,xmin= lwrB/1000,xmax=uprB/1000),linetype='dotted')+theme_test()+
...
###dist of params
ggplot(as.data.frame(model1),aes(b_Intercept,b_sE))+
  geom_hex(bins = 70) +
  scale_fill_continuous(type = "viridis") +
  theme_bw()
mod = as.data.frame(model)
...
dd = bind_rows(junk)
ggplot(dd,aes(b_Intercept,b_sE))+
  geom_hex(bins = 70) +
  scale_fill_continuous(type = "viridis") +
  theme_test(base_size=14)+ labs(x='Intercept',y='Slope')
...
#ddthin = dd[seq(1,nrow(dd),by=50),]
for(i in 1 :nrow(dd)) {
  y = dd[i,'b_Intercept']+dd[i,'b_I1DsE']*ests
  LRP_jp = c(LRP_jp, ests[which.min(abs(y-mL))])
  LRP_buff = c(LRP_buff, ests[which.min(abs(y-mL*1.25))])
}     
