...
ggplot(subset(ind35,year>=2000 &year<2024),aes(year,estB/1000,ymin=lwrB/1000,ymax=uprB/1000))+geom_point()+geom_ribbon(alpha=.25)+geom_path()+
  geom_ribbon(data=subset(ind35,year<2024),aes(year, estB/1000, ymin=lwrB/1000,ymax=uprB/1000),alpha=.25,fill='red')+
  geom_point(data=subset(ind35,year<2024),aes(year, estB/1000),pch=1)+geom_path(data=subset(ind35,year<2024),aes(year, estB/1000),linetype='dashed')+
...
gl = subset(gl,select=c(year,LFA36))

gg = merge(gl,g)
maxl =max(gg$LFA36)
maxv = max(gg$MEAN_T)
ggplot(subset(gg, year<2024), aes(x = year)) +
  geom_bar(aes(y =LFA36), stat = "identity", width = 0.75,alpha=.25) +
...
ggplot(as.data.frame(model),aes(b_Intercept,b_I1DsE))+
  ...
  scale_x_continuous(limits=c(0.03,.08))
theme_bw()
...
qua = quantile(xp$b_Intercept,c(0.025,0.975))
  qub = quantile(xp$b_I1DsE,c(0.025,0.975))
xp = subset(xp,b_Intercept>=qua[1] & b_Intercept<=qua[2] & b_I1DsE>=qub[1] & b_I1DsE<=qub[2])
...
ggplot(dd,aes(b_Intercept,b_I1DsE))+
  geom_hex(bins = 70) +
  scale_fill_continuous(type = "viridis") +
  scale_x_continuous(limits=c(0.02,.082))+
