...
l35 = gam(WEIGHT_KG~fYear+offset(leffort),data=subset(aT,LFA==35),family = Gamma(link='log'),method='REML')
l35a = gam(WEIGHT_KG~s(time)+fYear+offset(leffort),data=subset(aT,LFA==35),family = Gamma(link='log'),method='REML')
l35b = gam(WEIGHT_KG~s(time)+s(time,by=fYear)+fYear+offset(leffort),data=subset(aT,LFA==35),family = Gamma(link='log'),method='REML')

saveRDS(list(l35,l35a,l35b),'first3CPUEmodels35.rds')
w = readRDS('first3CPUEmodels35.rds')
l35 = w[[1]]
l35a = w[[2]]
l35b = w[[3]]

aT$rLID = as.factor(aT$LICENCE_ID)
l35c = gam(WEIGHT_KG~fYear+s(WOS,by=fYear)+s(rLID,bs='re')+offset(leffort),data=subset(aT,LFA==35),family = Gamma(link='log'),method='REML') #random intercept 
saveRDS(l35c,'CPUEmodel_LicenceRE35.rds')
l35c = readRDS('CPUEmodel_LicenceRE35.rds')


aT$rV = as.factor(aT$VR_NUMBER)
l35d = gam(WEIGHT_KG~fYear+s(time,by=fYear)+s(rV,bs='re')+offset(leffort),data=subset(aT,LFA==35),family = Gamma(link='log'),method='REML') # random intercept
saveRDS(l35d,'CPUEmodel_vesselRE35.rds')
l35d = readRDS('CPUEmodel_vesselRE35.rds')
...
ind = aggregate(SD_LOG_ID~time+SYEAR,data=subset(aT,LFA ==35),FUN=function(x) length(unique(x)))
...
b = emmeans::emmeans(l35,~fYear,offset=0,type='response',data=subset(aT,LFA==35))
bmm = as.data.frame(summary(b))[c('response', 'SE')]
bmm$Model = 'Base'
bmm$SYEAR = 2006:2023
names(bmm)[1:2]=c('wemm','wse')

d = emmeans::emmeans(l35a,~fYear+time,offset=0,type='response',data=subset(aT,LFA==35),at=list(time=c(1:60,160:292)))
...
f = emmeans::emmeans(l35b,~fYear+time,offset=0,type='response',data=subset(aT,LFA==35),at=list(time=c(1:60,160:292)))
...
g = emmeans::emmeans(l35c,~fYear+time,offset=0,type='response',data=subset(aT,LFA==35),at=list(time=c(1:60,160:292)))
